...
'###### used for drawing alphablend shadow ##############
...
Private Function PictureFromDC(ByVal hdcSrc&, ByVal nLeft&, ByVal nTop&, ByVal nWidth&, ByVal nHeight&) As StdPicture
'############################################################################
...
Private Function PictureFromBitmap(ByVal hBmp&, ByVal hPal&) As StdPicture
...
Me.Show 1
'NO FURTHER CODE IS EXECUTED UNTILL THE FORM IS Hidden
...
Private Function AlphaBlend1(RGB1 As Long, RGB2 As Long, Alpha As Integer, Optional Intensity As Integer) As Long
...
Const nHex As Long = &HFF
    ...
    'If Intensity = 0 Then Intensity = 75
...
'at this point the form must have been sized correctly and the balloon without
'shadow will have been created as a region.
...
Sub SetupButtons(ByVal MF As MsgBox_Flags, Optional b1$, Optional b2$, Optional b3$)
...
Private Sub F1_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
...
End Sub
 ...
 Private Sub TransparentBlt(DstDC, SrcDC, SrcRect As RECT, DstX, DstY, TransColor As Long)
  'eg
...
End Sub
Private Sub LoadIconID(Obj1 As Object, startcell As Integer, nWidth%, BKcol&)
...
'return the value of the button clicked + 100 if the checkbox was also selected
If Check2.Value = 1 Then iTxt = iTxt & "¬"
If nAction = -1 Then input_box = "" Else input_box = iTxt.Text
...
Sub SetDisplayIcon(MF As MsgBox_Flags, ID As Integer)
...
Private Sub iMsg_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    'allow the form mouse move event to decide if draging is enabled
    Form_MouseDown Button, Shift, X, Y
